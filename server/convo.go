...
func NewConversationManager(conversationID string, database *db.DB, baseLogger *slog.Logger, toolSetConfig claudetool.ToolSetConfig, integrationSkills *integrationSkillCache, recordMessage loop.MessageRecordFunc, recordTurnStartMessage turnStartRecordFunc, recordMessageBatch messageBatchRecordFunc, onStateChange func(ConversationState), streamPub *subpub.SubPub[StreamResponse]) *ConversationManager {
	...
	// recorded through this manager (loop turns and direct calls alike) so the
	// UI can resolve file references in a message against the cwd of its time;
	// later change_dir calls cannot re-point older references. The loop records
	// each assistant message before running its tools, and the user-driven
	// SetCwd refuses to move while a turn is in flight, so the cwd read here is
	// the one the message was written under. Cwd() takes cm.mu, which no
	// recordMessage caller holds.
