...
// waitForFfmpegExit polls until no ffmpeg process matching the session's
// output path is still running (pgrep exits non-zero when nothing matches).
func waitForFfmpegExit(t *testing.T, sessionID string) {
	t.Helper()
	deadline := time.Now().Add(5 * time.Second)
	for time.Now().Before(deadline) {
		if err := exec.Command("pgrep", "-f", sessionID+".mp4").Run(); err != nil {
			return
		}
		time.Sleep(10 * time.Millisecond)
	}
	t.Fatal("ffmpeg did not exit within 5s")
}

// waitForEncodedOutput polls until ffmpeg has encoded frames into the MP4
// (file size > 0 and stable across polls), so a subsequent SIGTERM finalizes
// a non-empty, playable file. Polling sc.frameCount is not enough: frames can
// sit in the stdin pipe long before ffmpeg encodes them.
func waitForEncodedOutput(t *testing.T, sessionID string) {
	t.Helper()
	deadline := time.Now().Add(5 * time.Second)
	var last int64
	for time.Now().Before(deadline) {
		info, err := os.Stat(filepath.Join(ScreencastDir, sessionID+".mp4"))
		if err == nil && info.Size() > 0 && info.Size() == last {
			return
		}
		if err == nil {
			last = info.Size()
	...
	t.Fatal("ffmpeg produced no output within 5s")
}
...
func TestScreencastSigtermYieldsPlayableMP4(t *testing.T) {
	...
	// Let ffmpeg actually encode frames into the MP4 before signaling, so the
	// finalize path has data to work with.
	waitForEncodedOutput(t, sessionID)
