...
func TestPatchToolExposesAndAcceptsSimpleInput(t *testing.T) {
	...
	result := tool.Run(t.Context(), json.RawMessage(`{"path":"simple.txt","oldText":"beta","newText":"B"}`))
	if result.Error != nil {
...
func TestSimplePatchRejectsCombinedModifications(t *testing.T) {
	...
	tool := (&PatchTool{WorkingDir: NewMutableWorkingDir(tempDir), Profile: "simple"}).Tool()
	result := tool.Run(t.Context(), json.RawMessage(`{"path":"simple.txt","oldText":"same","newText":"changed","append":"appended\n"}`))
...
func TestPatchToolExposesAndAcceptsComplexInput(t *testing.T) {
	...
	result := tool.Run(t.Context(), json.RawMessage(`{"path":"complex.txt","operation":"overwrite","newText":"hello"}`))
	if result.Error != nil {
		t.Fatal(result.Error)
	}
...
func TestSimplePatchFailureDoesNotWrite(t *testing.T) {
	...
	tool := (&PatchTool{WorkingDir: NewMutableWorkingDir(tempDir), Profile: "simple"}).Tool()
	result := tool.Run(t.Context(), json.RawMessage(`{"path":"simple.txt","oldText":"missing","newText":"M"}`))
	if result.Error == nil {
