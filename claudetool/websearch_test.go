...
func TestWebSearchRunStructuredBlocks(t *testing.T) {
	...
	out := tool.run(t.Context(), webSearchInput{Query: "test query"})
	if out.Error != nil {
...
func TestWebSearchArgsAndToolName(t *testing.T) {
	...
	n := uint64(3)
	out := tool.run(t.Context(), webSearchInput{Query: "  spaced query  ", NumResults: &n})
	if out.Error != nil {
		t.Fatalf("unexpected error: %v", out.Error)
	}
	...
	out = tool.run(t.Context(), webSearchInput{Query: "q"})
	if out.Error != nil {
		t.Fatalf("unexpected error: %v", out.Error)
	}
...
func TestWebSearchEmptyQuery(t *testing.T) {
	tool := &WebSearchTool{}
	out := tool.run(t.Context(), webSearchInput{Query: "  "})
	if out.Error == nil {
...
func TestWebFetchRun(t *testing.T) {
	...
	mc := uint64(500)
	out := tool.run(t.Context(), webFetchInput{URLs: []string{"https://example.com", "https://example.org"}, MaxCharacters: &mc})
	if out.Error != nil {
		t.Fatalf("unexpected error: %v", out.Error)
	}
...
func TestWebFetchNoURLs(t *testing.T) {
	tool := &WebFetchTool{}
	out := tool.run(t.Context(), webFetchInput{})
	if out.Error == nil {
...
// TestWebSearchRegistrationGating verifies the vendor-neutral WebSearch tool
// is registered for services WITHOUT native server-side search, and skipped
// when the service has it (where the native tool takes over). WebFetch is
// always present.
func TestWebSearchRegistrationGating(t *testing.T) {
	...
	got := names(ts)
	if !got["WebSearch(false)"] || !got["WebFetch(false)"] {
	...
	// Capable service -> no client WebSearch (native present), WebFetch still there.
	...
	got = names(ts)
	if got["WebSearch(false)"] {
		t.Errorf("capable service should not get client WebSearch: %#v", got)
	}
	if !got["WebFetch(false)"] {
		t.Errorf("capable service missing WebFetch: %#v", got)
	}
	...
	ts = NewToolSet(t.Context(), ToolSetConfig{WorkingDir: "/test"})
	got = names(ts)
	if !got["WebSearch(false)"] || !got["WebFetch(false)"] {
